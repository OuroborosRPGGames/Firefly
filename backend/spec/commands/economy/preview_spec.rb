...
RSpec.describe Commands::Economy::Preview do
  ...
  def execute_command(args = nil)
    input = args.nil? ? 'preview' : "preview #{args}"
    command.execute(input)
  end

  describe 'command metadata' do
    it 'has correct command_name' do
      expect(described_class.command_name).to eq('preview')
    end
  ...
  describe '#execute' do
    context 'without arguments' do
      it 'returns error requesting target' do
